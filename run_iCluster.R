...
source(sprintf("%s/iCluster.R", code_path))
...
if (file.exists(sprintf("%s/state_cohort_%s_pathway_%s_iCluster_%s.RData", result_path, cohort, pathway, cluster_count)) == FALSE) {
  ...
  state <- iCluster(K_train, parameters)
  save("state", file = sprintf("%s/state_cohort_%s_pathway_%s_iCluster_%s.RData", result_path, cohort, pathway, cluster_count))  
}
